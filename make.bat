...
REM bin generates the releaseable binaries for GOBBM
:bin
	call :generate
	call .\scripts\windows\build.bat "%CD%"
	goto :eof

REM dev creates binaries for testing GOBBM locally. These are put
REM into ./bin/ as well as %GOPATH%/bin
:dev
	call :generate
	call .\scripts\windows\build.bat "%CD%" GOBBM_DEV
	goto :eof
...
:vet
	...
	set _vetExitCode=0
	set _GOBBM_PKG_DIRS=%TEMP%\gobbm-pkg-dirs.txt

	go list -f {{.Dir}} ./... >"%_GOBBM_PKG_DIRS%"
	REM Skip the first row, which is the main gobbm package (.*github.com/patdhlk/gobbm$)
	for /f "delims= skip=1" %%d in ("%_GOBBM_PKG_DIRS%") do (
		go tool vet %_VETARGS% "%%d"
		if ERRORLEVEL 1 set _vetExitCode=1
		call :setMaxExitCode %_vetExitCode%
	)
	del /f "%_GOBBM_PKG_DIRS%" 2>NUL
