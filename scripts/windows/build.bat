...
if x%2 == xGOBBM_DEV set _DEV_BUILD=1

cd %1
md bin 2>nul

:: Get the git commit
set _GIT_COMMIT_FILE=%TEMP%\gobbm-git_commit.txt
set _GIT_DIRTY_FILE=%TEMP%\gobbm-git_dirty.txt

set _NUL_CMP_FILE=%TEMP%\gobbm-nul_cmp.txt
...
set _GODEP_PATH_FILE=%TEMP%\gobbm-godep-path.txt
...
set _GO_ENV_TMP_FILE=%TEMP%\gobbm-go-env.txt
...
gox^
 -os="%_XC_OS%"^
 -arch="%_XC_ARCH%"^
 -ldflags "-X github.com/patdhlk/gobbm/version.GitCommit=%_GIT_COMMIT%%_GIT_DIRTY%"^
 -output "pkg/{{.OS}}_{{.Arch}}/gobbm"^
 .
